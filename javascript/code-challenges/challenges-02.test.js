...
in an array of numbers and returns a new array of
...
const addOne = (arr) => {
  // Solution code here...
  const num = arr.map(x=>x + 1);
  return num;
};

/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
Write a fud anction nameddQuestion that, given an array of strings,
...
const charCode = (arr) => {
  // Solution code here...
  return arr.map(st => st.charCodeAt());
};
...
const extractAbilities = (arr) => {
  // Solution code here...
  const abilities = [];
  arr.map(key => abilities.push(key.ability.name));
  return abilities;
};
