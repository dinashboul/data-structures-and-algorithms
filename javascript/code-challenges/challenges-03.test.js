...
Write a function named containsAnd that, given an array of strings as input, uses filter to return an array containing
...
const containsAnd = (arr) => {
  // Solution code here...
  const result =arr.filter(item =>item.includes('and'));
  return result;
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Solution code here...
  const result=arr.filter(item=>item.baseStat>minBaseStat);
  return result;
};
