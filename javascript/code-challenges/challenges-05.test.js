...
const addValues = (arr) => {
  // Solution code here...
  const value = arr.reduce((item, endValue) => item+endValue,0);
  return value;
};
...
Each object contains the keys `item` and `purchasePrice`
 like the example.
...
const addPurchases = (arr) => {
  // Solution code here...
  let Sum = arr.reduce((item, currValue) => {
    return item + currValue.purchasePrice;}, 0);
  return(Sum);
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  return(arr.reduce((startValue, endValue, index) => {
    startValue + endValue;
    return index+1;
  }));
};
...
const returnNames = (arr) => {
  // Solution code here...
  const newAraay = [];
  arr.reduce((startValue, endValue) => {
    newAraay.push(endValue.name);
  });
  newAraay.unshift(starWarsData[0].name);
  return newAraay;
};
...
const reversedString = (str) => {
  // Solution code here...
  let word =str.split("");
  return (word.reduce((rev, char)=> char + rev, ''));
};
