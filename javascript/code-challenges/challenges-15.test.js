...
Write a function named screenForNames 
that takes in an array of strings and uses Regex
 to create a new array of only those strings that 
...
const screenForNames = (arr) => {
  ...
  let str= arr.filter((item)=>{
   let word =/^\b(Mr.|Mrs.|Ms.|Dr.)\s\w+$/g;
    return  word.test(item);
  });
 return str ;
};
...
For example, ['apple', 'banana', 'MacGyver']
 returns ['Apple', 'Banana', 'MacGyver'].
------------------------------------------------------------------------------------------------ */
...
Write a function named biggerThanLuke that,
 ...
 returns the names of the characters whose mass is
  greater than Luke's.
